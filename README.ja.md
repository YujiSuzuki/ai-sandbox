...
対話形式でインストールと設定ファイルの用意を行います。完了後に表示される案内に従って `hostmcp serve --workspace '<path>'` を実行してください（ホストツールを使う場合は `--sync` を付けます — 詳細は下記。このターミナルは開いたままにします）。既にインストール済みの状態でもう一度実行すると、更新チェックも行われます。

...
セットアップの詳細は [ai-sandbox-demo README](https://github.com/YujiSuzuki/ai-sandbox-demo) を参照してください。
...
### デモアプリ起動後（ai-sandbox-demo）：

```
「securenote-api のログを見せて」
→ HostMCP 経由でコンテナログが表示される
...
- HostMCP の設定（`hostmcp.example.yaml`）で `allowed_containers` にコンテナ名パターンが含まれているか確認

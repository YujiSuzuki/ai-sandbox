...
> **Related tool: firejail**
>
> [firejail](https://firejail.wordpress.com/) is another tool that often comes up on Linux. It uses Linux namespaces to sandbox a single **process**, putting it in the same category as bubblewrap (which Claude Code Sandboxing uses) — unlike this project or Docker AI Sandboxes, which carve out a whole separate environment for the entire AI agent, firejail can be nested inside a larger sandbox (such as this project's container) rather than replacing it.
>
> That said, on the specific question of "hiding secrets" — this project's core focus ([Gap 1](#gap-1-filesystem-level-secret-hiding)) — the comparison looks different. firejail's `--blacklist` option denies access to the target path at the OS kernel level; per [firejail's own docs](https://man.archlinux.org/man/firejail.1.en), the file "is still visible on the filesystem, even if it's inaccessible" — completely inaccessible, but the exact implementation (whether it's a bind-mount or something else) isn't documented publicly. Either way, since it isn't dependent on the AI tool respecting a rule, it's closer in spirit to this project's volume-mount approach than to a deny rule — though the specific behavior (this project's files appear empty to the AI, rather than merely inaccessible) may differ.
>
> Where firejail still falls short of this project: it's a single-process restriction, not something designed to run as a unit across a whole multi-project workspace, and it doesn't give controlled access to other containers (the area HostMCP covers). It's also Linux-only, whereas macOS is this project's primary target (the main reasons for choosing Docker here were cross-platform support and integration with VS Code's "Reopen in Container" workflow). So on a Linux dev machine, firejail's `--blacklist` could plausibly replicate the secret-hiding part on its own — just not the rest (multi-project scope, controlled cross-container access).
...
- **AWS Bedrock AgentCore Code Interpreter**: spins up a dedicated Firecracker microVM per session — a one-session-one-microVM model ([AWS docs](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/code-interpreter-session-characteristics.html))

...
> [!TIP]
> macOSの場合、`install-hostmcp.sh` がプロジェクトルートに `--workspace` を埋め込み済みのダブルクリック起動用ランチャー `hostmcp-sync.command` を作成します。AIから「新しくステージングされたホストツールの承認が必要です」と言われたら、ターミナルで上記コマンドを打ち直す代わりにこれをダブルクリックできます。
...
host_access:
  host_tools:
    ...
    timeout: 60  # 秒。独自のタイムアウトを宣言していないツールに使われる
...
グローバルな `timeout`（デフォルト60秒）を常に超える処理が必要な場合は、全ツール共通のデフォルト値を引き上げる代わりに、ヘッダーに `@timeout:` 行を追加して個別にタイムアウトを宣言できます — 上記の[設定](#設定)や、実例としての `xcode-test.sh` を参照してください:
...
- **タイムアウト** — ツールの実行にはタイムアウトがあり（デフォルト: 60 秒）、暴走スクリプトを防止します。ツールはヘッダーで（`# @timeout: <秒数>`）より長いタイムアウトを自ら宣言できますが、有効になるのは `hostmcp tools sync` で承認された後だけです。承認プロンプトは `@timeout:` 宣言があれば確認を求める前に必ず表示するため、人間のレビュアーが気づかないまま承認することはできません。宣言値は `max_tool_timeout` でもクランプされ、これは管理者が `hostmcp.yaml` を編集して初めて引き上げられます（スクリプト側で大きな値を宣言するだけでは上限を回避できません）。

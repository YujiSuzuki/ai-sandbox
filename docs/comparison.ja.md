...
> **関連ツール：firejail**
>
> [firejail](https://firejail.wordpress.com/)もLinux上でよく比較に挙がる。firejailはLinux namespaceを使って**プロセス単位**でサンドボックス化する方式で、Claude Code Sandboxingが使うbubblewrapと同じカテゴリに属する——本プロジェクトやDocker AI Sandboxesのように「AIエージェント全体の実行環境を丸ごと切り出す」方式とは異なり、より大きいサンドボックス（本プロジェクトのコンテナなど）の内側にネストして使うことができる。
>
> ただし「シークレットを隠す」というこのプロジェクトの主眼（[ギャップ1](#ギャップ1-ファイルシステムレベルのシークレット隠蔽)）に限れば話が変わる。firejailの`--blacklist`オプションは、対象パスへのアクセスをOSカーネルレベルで拒否する仕組みで、[firejail自身のドキュメント](https://man.archlinux.org/man/firejail.1.en)によれば対象ファイル・ディレクトリは「ファイルシステム上には見え続けるが、アクセス不能になる」と説明されている——ここでの「アクセス不能」は完全な不能を指す。正確な実装（bind-mountによるものかどうか等）は公開ドキュメントからは確認できなかったが、少なくとも「AIツールがルールを尊重するかどうかに依存しない」という点では、アプリケーションレベルのdenyルールよりも本プロジェクトのボリュームマウント方式に近い性質を持つ。ただし本プロジェクトの方式（AIから見て空ファイル・空ディレクトリになる）とは、見え方が異なる可能性がある点には注意が必要。
>
> 一方でfirejailは単一プロセスの制限にとどまり、マルチプロジェクトワークスペース全体を1つの単位として扱う設計ではなく、他コンテナへの制御されたアクセス（HostMCPが担う領域）も対象外。またLinux専用で、本プロジェクトが主要ターゲットとするmacOSでは使えない（Dockerを選んだ主な理由は、クロスプラットフォーム対応とVS Codeの「Reopen in Container」ワークフローとの統合）。そのためLinux開発機に限れば、firejailの`--blacklist`だけでシークレット隠蔽の部分を独自に再現できる可能性はあるが、それ以外（マルチプロジェクトスコープ、コンテナ間アクセス制御）はカバーしない。

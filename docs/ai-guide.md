...
│   │   ├── sync-secrets.py           # 🐳 Interactive secret sync tool
│   │   ├── sync-compose-secrets.py   # 🐳 Sync between DevContainer/CLI compose
│   │   ├── merge-claude-settings.py  # Merge subproject .claude/settings.json
...
`check-undeclared-secrets.py` (heuristic scan) only reports files that look like secrets but aren't hidden in `docker-compose.yml`; it never determines whether a finding is a real secret. To act on those findings, run `triage-undeclared-secrets.py`, which walks through them one at a time and lets the user judge each — hide it via `docker-compose.yml` if it's a real secret, or add it to `sync-ignore` if it's not — and never writes without that per-item confirmation.

...
if [[ "${LANG:-}" == ja_JP* ]] || [[ "${LC_ALL:-}" == ja_JP* ]]; then
    ...
    MSG_DISCLAIMER="名前パターンによる推測です。誤検知があり得ます -- 1件ずつ内容を確認してください。"
    ...
    MSG_ACTION3="  秘密でないなら: このメッセージは無視してよい（誤検知）"
    ...
    MSG_DISCLAIMER="This is a name-pattern guess and can produce false positives -- review each one."
    ...
    MSG_ACTION3="  If not secret: this is a false positive, safe to ignore"
